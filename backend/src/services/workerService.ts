...
import { extractSignature, getWrapperTemplate, claimSignatureLock, updateSignatureWrapper, waitForSignatureWrapper } from './signatureService';
    ...
    if (submission.language === 'python' && pythonAnalyzer.isComplexityMode(submission.code)) {
        ...
        if (wrapperTemplate) {
            ...
            try {
                ...
                await claimSignatureLock(submission.language, sig);
                ...
                await updateSignatureWrapper(submission.language, sig, wrapperTemplate);
            } catch (err: any) {
                if (err.code === '23505') { // Postgres unique_violation code
                    console.log(`[WAITING CACHE] Another submission is generating wrapper for ${sig}`);
