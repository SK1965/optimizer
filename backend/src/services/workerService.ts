...
import { extractSignature, getWrapperTemplate, waitForSignatureWrapper, releaseSignatureLock } from './signatureService';
...
export const processSubmission = async (id: string) => {
  try {
    ...
    if (submission.language === 'python' && pythonAnalyzer.isComplexityMode(submission.code)) {
        ...
        if (wrapperTemplate) {
            console.log(`[CACHE HIT] signature=${sig}`);
        } else {
            console.log(`[CACHE MISS] signature=${sig}`);
            ...
            try {
                ...
                await client.query(
                    `INSERT INTO signature_wrappers (id, language, normalized_signature, wrapper_template) VALUES ($1, $2, $3, NULL)`,
                    [randomUUID(), submission.language, sig]
                );
                ...
                await client.query(
                    `UPDATE signature_wrappers SET wrapper_template = $1 WHERE language = $2 AND normalized_signature = $3`,
                    [wrapperTemplate, submission.language, sig]
                );
                ...
                if (err.code === '23505') { // Postgres unique_violation code
                    console.log(`[WAITING FOR WRAPPER] signature=${sig}`);
                    wrapperTemplate = await waitForSignatureWrapper(submission.language, sig);
                } else {
                    console.log(`[LOCK CLEANUP] signature lock released due to failure`);
                    await releaseSignatureLock(submission.language, sig);
                    throw err;
                }
            } finally {
                client.release();
